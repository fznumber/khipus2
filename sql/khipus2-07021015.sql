...
(
    IDTERRITORIOTRABAJO BIGINT PRIMARY KEY NOT NULL,
    NOMBRE VARCHAR(100),
    PAIS VARCHAR(50),
...
CREATE UNIQUE INDEX unique_secuencia ON codigopedidosecuencia (secuencia);
...
ALTER TABLE persona ADD FOREIGN KEY (IDTERRITORIOTRABAJO) REFERENCES territoriotrabajo (IDTERRITORIOTRABAJO);
CREATE UNIQUE INDEX IDPERSONA_UNIQUE ON persona (IDPERSONA);
CREATE INDEX FK_PERSONA_TERRITORIO_idx ON persona (IDTERRITORIOTRABAJO);
...
ALTER TABLE territoriotrabajo ADD FOREIGN KEY (IDDEPARTAMENTO) REFERENCES departamento (IDDEPARTAMENTO);
ALTER TABLE territoriotrabajo ADD FOREIGN KEY (IDDISTRIBUIDOR) REFERENCES persona (IDPERSONA);
CREATE INDEX fk_territorio_departamento_idx ON territoriotrabajo (IDDEPARTAMENTO);
CREATE INDEX fk_territorio_persona_idx ON territoriotrabajo (IDDISTRIBUIDOR);
...
ALTER TABLE ventaarticulo ADD FOREIGN KEY (IDPROMOCION) REFERENCES promocion (IDPROMOCION);
...
CREATE INDEX FK_ventaarticulo_promocion_idx ON ventaarticulo (IDPROMOCION);
