...
/*ALTER TABLE ARTICULOS_PEDIDO ADD CONSTRAINT ARTICULOS_PEDIDO_PK PRIMARY KEY ( IDARTICULOSPEDIDO ) ;*/
 ...
 CREATE TABLE `eos`.`id_gen` (
  `GEN_NAME` VARCHAR(30) NOT NULL,
  `GEN_VAL` BIGINT(50) NULL,
...
CREATE
 ...
 CREATE TABLE tiporetencion
  (
    idtiporetencion BIGINT (24) NOT NULL ,
    descripcion     VARCHAR (100),
    PI_ID           BIGINT (24) NOT NULL ,
    idretencion     BIGINT (24) NOT NULL ,
    PRIMARY KEY ( idtiporetencion )
  );
ALTER TABLE tiporetencion ADD CONSTRAINT tiporetencion_CLIENTE_FK FOREIGN KEY (PI_ID) REFERENCES CLIENTE (PI_ID);
ALTER TABLE tiporetencion ADD CONSTRAINT tiporetencion_RETENCION_FK FOREIGN KEY(idretencion) REFERENCES RETENCION (IDRETENCION);
...
CREATE TABLE VentaCliente
  (
    ...
    CONSTRAINT VentaCliente_PK PRIMARY KEY ( IDVENTACLIENTE )
  );
  drop table VentaCliente;
ALTER TABLE VentaCliente ADD CONSTRAINT VentaCliente_CLIENTE_FK FOREIGN KEY (IDPERSONA ) REFERENCES persona ( IDPERSONA );
ALTER TABLE VentaCliente ADD CONSTRAINT VentaCliente_VENTAARTICULO_FK FOREIGN KEY ( IDVENTAARTICULO ) REFERENCES VENTAARTICULO ( IDVENTAARTICULO );
  ...
  DELETE CASCADE ;  
  ALTER TABLE `eos`.`articulos_pedido` ADD INDEX `ARTICULOS_PEDIDO_INV_ARTICULOS_FK_idx` (`COD_ART` ASC, `NO_CIA` ASC);
  ALTER TABLE `eos`.`articulos_pedido` ADD CONSTRAINT `ARTICULOS_PEDIDO_INV_ARTICULOS_FK`  FOREIGN KEY (`COD_ART` , `NO_CIA`)  REFERENCES `eos`.`inv_articulos` (`COD_ART` , `NO_CIA`)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION;
